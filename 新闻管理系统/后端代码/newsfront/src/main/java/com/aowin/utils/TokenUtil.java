...
public class TokenUtil {
	...
	public void init() {
		tokenUtil = this;
		tokenUtil.customerMapper = this.customerMapper;
	}
	...
	public static String createToken(String id, String username) {
		...
		JwtBuilder builder = Jwts.builder().setHeader(header).setId(id)
				.setExpiration(new Date(System.currentTimeMillis() + EXPIRE_TIME)).setSubject(username)
	...
	public static int verify(String token, HttpServletRequest req) {
		...
		try {
			...
			Customer login = tokenUtil.customerMapper.selectCustomerByAccount(id);
			System.out.println("login:"+login);
			if (login != null) {
				req.setAttribute("login", login.getAccount());
				return 1;
			} else {
				return 0;
			}
